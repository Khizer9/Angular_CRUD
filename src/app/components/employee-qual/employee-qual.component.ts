...
export class EmployeeQualComponent implements OnInit {
  ...
  addQual() {
    ...
    if (!this.isEdit) {
      if (this.employeeService.employeesArray[this.qualObj.index]) {
        // if (!this.employeeService.employeesArray[index].QualArray) {
        //   this.employeeService.employeesArray[index].QualArray = [];
        // }
      ...
      if (this.qualObj.index >= 0) {
        this.employeeService.employeesArray[this.qualObj.index].QualArray[this.qualIndex] = {
          qual: this.type.controls.qual.value,
          year: this.type.controls.year.value,
          institute: this.type.controls.institute.value,
        };
  ...
  deleteQual(index: number) {
    if (this.employeeService.employeesArray[this.qualObj.index].QualArray.length >= 0) {
      this.employeeService.employeesArray[this.qualObj.index].QualArray.splice(index, 1);
    } else {
      console.log('Error');
    }
  }
